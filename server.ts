...
const db = new Database("mkanak.db");
...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS transfers (
    ...
    type TEXT DEFAULT 'send', -- 'send' or 'request'
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
...
async function startServer() {
  ...
  app.post("/api/auth/login", (req, res) => {
    ...
    const user = db.prepare("SELECT u.*, b.name as branch_name FROM users u LEFT JOIN branches b ON u.branch_id = b.id WHERE username = ? AND password = ?").get(username, password) as any;
    if (user) {
  ...
  app.get("/api/dashboard/stats", (req, res) => {
    const stats = {
      totalSales: db.prepare("SELECT SUM(total_amount) as total FROM sales").get() as any,
      totalRevenue: db.prepare("SELECT SUM(total_amount) as total FROM sales WHERE created_at >= date('now')").get() as any,
      lowStock: db.prepare("SELECT COUNT(*) as count FROM inventory i JOIN products p ON i.product_id = p.id WHERE i.quantity <= p.min_stock").get() as any,
  ...
  app.get("/api/products", (req, res) => {
    ...
    if (branch_id) {
      ...
      // Super admin view: all products with total stock
      const products = db.prepare(`
  ...
  app.post("/api/products", (req, res) => {
    ...
    try {
      const transaction = db.transaction(() => {
        ...
        // Add initial stock to the specified branch (usually main warehouse)
  ...
  app.post("/api/transfers", (req, res) => {
    ...
    try {
      const transaction = db.transaction(() => {
        ...
        // If I 'send' to you, notify you (to_branch_id)
        // If I 'request' from you, notify you (from_branch_id)
        const notifiedBranchId = type === 'send' ? to_branch_id : from_branch_id;
        const creatorBranchId = type === 'send' ? from_branch_id : to_branch_id;

        const creatorBranch = db.prepare("SELECT name FROM branches WHERE id = ?").get(creatorBranchId) as any;
    ...
    try {
      const transaction = db.transaction(() => {
        ...
        for (const item of items) {
          ...
          `).get(item.product_id, transfer.from_branch_id) as any;
          ...
          // Update destination inventory (increase/upsert)
          const destStock = db.prepare(`
            SELECT id FROM inventory WHERE product_id = ? AND branch_id = ?
          `).get(item.product_id, transfer.to_branch_id) as any;
        ...
        // If it was a 'send', notify the sender (from_branch_id)
        // If it was a 'request', notify the requester (to_branch_id)
        ...
        const acceptorBranch = db.prepare("SELECT name FROM branches WHERE id = ?").get(acceptorBranchId) as any;
        db.prepare(`
          INSERT INTO notifications (branch_id, title, message, type)
          VALUES (?, ?, ?, ?)
        `).run(
  ...
  app.post("/api/transfers/:id/reject", (req, res) => {
    ...
    try {
      ...
      const rejectorBranch = db.prepare("SELECT name FROM branches WHERE id = ?").get(rejectorBranchId) as any;
  ...
  app.post("/api/messages", (req, res) => {
    ...
    try {
      ...
      // Add notification for the recipient branch
      const fromBranch = db.prepare("SELECT name FROM branches WHERE id = ?").get(from_branch_id) as any;
      db.prepare(`
        INSERT INTO notifications (branch_id, title, message, type)
        VALUES (?, ?, ?, ?)
  ...
  app.get("/api/reports/stats", (req, res) => {
    try {
      ...
      const totalTax = db.prepare("SELECT SUM(tax) as total FROM sales").get() as any;
      const totalOrders = db.prepare("SELECT COUNT(*) as count FROM sales").get() as any;
